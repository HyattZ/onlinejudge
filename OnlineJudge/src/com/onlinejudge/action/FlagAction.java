...
public class FlagAction implements ModelDriven, SessionAware {
	...
	public String checkFlag() {
		...
		if (flagFormBean.validateFlag() && flagFormBean.validateProblemid()) {
			// 如果数据不非法
			Problem problem = problemService.getProblemByProblemid(flagFormBean
					.getProblemid());
			// 判断这道题目是不是已经做过
			if (!recordService.isComplete((Integer) session.get("stuid"),
				...
				if (submitTimesService.checkSubmittimes(
					...
					if (problem.getFlag().trim()
						...
						//题目正确对周榜进行更新
						boolean flag0 = weeklyScoreService.updateScore((Integer) session.get("stuid"),
								problem.getMark());
						...
						if (!flag) {
							...
							if (!recordService.saveRecord((Integer) session.get("stuid"),
									flagFormBean.getProblemid(),flagFormBean.getFlag())){
								session.put("errorReason", "写入数据库出错，请再次提交！");
								session.put("errorPageTitle", "系统错误");
								return Status.UPDATEFAIL;
							}
